...
mostrarJogadores(vencedoresChuteiraDeOuro);

input.addEventListener('input',()=>{
    let termo = input.value.toLowerCase().trim();
    let filtrados = vencedoresChuteiraDeOuro.filter(c=>c.nome.toLowerCase().includes(termo) || c.clube.toLowerCase().includes(termo) || c.pais.toLowerCase().includes(termo) || c.ano.toString().includes(termo));
